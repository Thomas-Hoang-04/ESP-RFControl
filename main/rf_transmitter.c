...
esp_err_t rf_init(gpio_num_t tx_gpio, int8_t repeat_count, Protocol* proto, RFTransmitter* rf_rmt) {
    ESP_RETURN_ON_FALSE(rf_rmt && proto, ESP_ERR_INVALID_ARG, TAG, "Invalid RF transmitter");
...
esp_err_t rf_deinit(RFTransmitter* rf_rmt) {
    ESP_RETURN_ON_FALSE(rf_rmt && rf_rmt->init, ESP_ERR_INVALID_ARG, TAG, "Invalid RF rf_rmt");
    if (rf_rmt->pulses) {
        free(rf_rmt->pulses);
        rf_rmt->pulses = NULL;
    }
...
esp_err_t rf_send(RFTransmitter* rf_rmt) {
    ...
    if (rf_rmt->active) {
        ESP_LOGW(TAG, "RF transmitter is busy");
